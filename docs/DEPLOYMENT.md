...
| `BLOB_STORE_ID` | M in prod | ✅ done | Vercel Blob for `media`, OIDC — set by connecting the store, see §4 |
| `BLOB_READ_WRITE_TOKEN` | O (legacy) | ✅ done | Fallback for hosts outside Vercel only, see §4 |
...
**Vercel Blob**, through a **custom OIDC adapter** —
`src/backend/payload/vercel-blob-oidc.ts`, wired in
`src/backend/payload/storage.ts` on top of `@payloadcms/plugin-cloud-storage`
and registered as a plugin in `src/payload.config.ts`.

Chosen over Cloudflare R2 and S3 because it adds no second vendor, no
second bill and no second credential to rotate: Vercel supplies the
credential itself. R2 is cheaper at high image bandwidth and remains the
sensible migration later — swapping the adapter is a config change, not a
schema change, so that door stays open.

#### Why not `@payloadcms/storage-vercel-blob`

Vercel Blob connections authenticate by **OIDC**: connecting a store sets
`BLOB_STORE_ID`, and the runtime issues a short-lived token per request, so
no long-lived `vercel_blob_rw_…` token exists. Payload's own package cannot
use that. Every published version — 3.88.0, 3.89.0 and the 4.0 previews —
pins `@vercel/blob` at 2.3.1, which predates OIDC (added in 2.4.0), derives
the store id by regex from the token string, and **disables itself entirely
when no token is set**, at which point uploads fall back to the ephemeral
disk without a word.

Our adapter is the same shape minus the token. It uses the same shared
`getFileKey`/`getFilePrefix` helpers, so stored paths and the collection
schema are unchanged, and it calls `put`/`del`/`head` from `@vercel/blob`
with **no token**, which is precisely what selects OIDC. Revisit this if
Payload ships OIDC support: deleting the adapter and returning to the vendor
plugin would be a small, contained change.

### Environment variables

Production needs **one** of these. `@vercel/blob` resolves credentials in a
fixed order: an explicit token, then OIDC (`BLOB_STORE_ID` plus the
runtime's token), then `BLOB_READ_WRITE_TOKEN`.

| Variable | | Notes |
| --- | --- | --- |
| `BLOB_STORE_ID` | M in prod, P | **Server-side only — never `NEXT_PUBLIC_`.** Set automatically once a Blob store is connected to the project. This is the credential on Vercel; nothing to create or paste. |
| `BLOB_READ_WRITE_TOKEN` | O (legacy) | Long-lived read/write token, for running **outside** Vercel (CI, another host). Not required for the Vercel deployment. An explicit token beats OIDC, so set it only to bypass OIDC deliberately. |

Both are declared in `src/lib/env.ts` and listed in `.env.example`, and the
production-runtime check requires one of them. Neither is required during
`next build` (the build makes no network calls), so a build never needs a
runtime credential.

### Behaviour per environment

| | Storage | With no credential |
| --- | --- | --- |
| **Local** | `./uploads` on disk | Expected. Logs an explicit warning naming the fallback. |
| **Preview** | Vercel Blob (OIDC) | Falls back to local disk. Acceptable — preview data is disposable. |
| **Production** | Vercel Blob (OIDC) | **Refuses to boot.** `resolveStorageMode()` throws rather than writing to an ephemeral disk. |
...
function and is unit-tested across these cases.

> **OIDC cannot be tested from local development.** The credential is issued
> by the Vercel runtime and exists only inside a deployment, so locally there
> is nothing to authenticate with and uploads go to `./uploads` by design.
> Verify blob storage on a **preview deployment**: upload an image in `/cms`,
> confirm it saves and renders, and confirm it survives a redeploy. If the
> SDK finds no credential it throws "No blob credentials found", which
> appears in the function logs.
>
> Preview and Production currently share the same store, so preview uploads
> write real objects into it.
...
> bypass the body limit. It is deliberately **off**, and our OIDC adapter
> does not implement it: the vendor version defaulted to
> `access: ({ req }) => !!req.user`, which would let any signed-in
> *customer* mint an upload token, and hardcoded `allowOverwrite: true` on a
> caller-chosen pathname. The correct access rule is written down as
> `clientUploadAccess` in `storage.ts`; enabling it means adding a browser
> upload path to the adapter and testing it against a live Blob store.
